...
class DarcyProblem:
    ...
    def addVelocityComponentBC(self, value, component, onboundary):
        ...
        self.bcs_u.append(
            (value, boundaryNameToIndex(onboundary), vectorComponentNameToIndex(component))
        )
...
class DarcySolver:
        ...
        if method == "sdhm":
            ...
            for uboundary, iboundary, component in bcs_u:
                if component is not None:
                    ...
                    self.bcs.append(
                        DirichletExpressionBC(
                            self._W.sub(0), rho * uboundary, iboundary, method=dirichlet_method
                        )
                    )
    ...
    def cgls_form(self, problem, mesh, bcs_p):
        ...
        a += (
            delta_1
            * inner(kappa * (inv_kappa * q + grad(p)), delta_0 * inv_kappa * w + grad(v))
            * dx
        )
    ...
    def dgls_form(self, problem, mesh, bcs_p):
        ...
        a += (eta_q * h_avg) * avg(inv_kappa) * (jump(q, n) * jump(w, n)) * dS + (
            eta_p / h_avg
        ) * avg(kappa) * dot(jump(v, n), jump(p, n)) * dS
