...
def test_chemicaltransport_step(num_regression):
    ...
    species_names_for_output = ['Ca++', 'Mg++', 'Calcite', 'Dolomite', 'CO2(aq)', 'HCO3-', 'Cl-', 'H2O(l)']
    ...
    species_data = { "n({})(step={})".format(name, i) : u for i, (name, u) in enumerate(zip(species_names_for_output, species_data)) }
    element_data = { "b({})(step={})".format(name, i) : u for i, (name, u) in enumerate(zip(element_names_for_output, element_data)) }
