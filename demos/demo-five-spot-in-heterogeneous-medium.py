...
cfl = 0.1        # the CFL number to be used in the calculation of time step
...
v_left = rok.conditional(y < h_well, 1e-3, 0)
v_bottom = rok.conditional(x < h_well, 1e-3, 0)
v_right = rok.conditional(Ly - y < h_well, 1e-8, 0)
v_top = rok.conditional(Lx - x < h_well, 1e-8, 0)

# Pressure BC?
# p_left = rok.conditional(y < h_well, 1e6, 0)
# p_bottom = rok.conditional(x < h_well, 1e6, 0)
# p_right = rok.conditional(Ly - y < h_well, 5e5, 0)
# p_top = rok.conditional(Lx - x < h_well, 5e5, 0)

# *** Nunez pressure BC below
# p_bottom = rok.utils.PressureFixBC(V, [Lx, 0], 0, 1)
# p_top = rok.utils.PressureFixBC(V, [0, Ly], 0, 1)
p_bottom = rok.conditional(Lx - x < h_well, 1e6, 0)
p_top = rok.conditional(Lx - x < h_well, 5e5, 0)
